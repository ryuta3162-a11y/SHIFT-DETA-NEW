...
/** 初回ロード: ログインユーザー・権限店舗・設定 */
function getBootstrap() {
  var email = requireUserEmail_();
  var domain = String(getSetting_('company_domain', '') || '').toLowerCase();
  if (domain) {
    var at = email.indexOf('@');
    var userDomain = at >= 0 ? email.substring(at + 1).toLowerCase() : '';
    if (userDomain !== domain) {
      throw new Error('このアプリは ' + domain + ' のアカウントのみ利用できます。');
    }
  }

  var acl = resolveAcl_(email);
  var stores = listStoresForUser_(acl);
  var profile = findEmployeeByEmail_(email);
  return {
    ok: true,
    email: email,
 ...
 */
function getShifts(storeId, yearMonth) {
  var email = requireUserEmail_();
  var acl = resolveAcl_(email);
...
/**
 * シフト1件の作成／更新
 * payload: { shift_id?, date, employee_id, store_id, status, start_time?, end_time?, break_minutes? }
 */
...
/** シフト1件削除 */
function deleteShift(shiftId, storeId) {
  var email = requireUserEmail_();
  var acl = resolveAcl_(email);
...
function requireUserEmail_() {
  ...
  // フォールバック（設定で許可したときだけ）: 実行ユーザー＝自分 の試験用
  // 設定シート key=allow_effective_user_fallback / value=TRUE
  if (effective && isTruthy_(getSetting_('allow_effective_user_fallback', 'FALSE'))) {
    return effective;
  }

  throw new Error(
    'ログインメールを取得できません。次を確認してください。\n' +
      '1) デプロイ → デプロイを管理 → 編集\n' +
      '　・実行ユーザー: 「ウェブアプリにアクセスしているユーザー」\n' +
      '　・アクセス: 「組織内の全員」\n' +
      '2) 「新しいバージョン」でデプロイし直す\n' +
      '3) 会社のGoogleアカウントでログインした状態で /exec を開く\n' +
      '（いま active空 / effective=' + (effective || '空') + '）'
  );
}
